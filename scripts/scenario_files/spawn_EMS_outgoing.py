...
class EMSOutgoing():
    """
    Represents a scenario where an EMS vehicle is departing a hospital and ego vehicle should yield this EMS vehicle.

    This scenario spawns one actor and is triggered when the ego vehicle enters a specified trigger area.
    The objective of the scenario is to test ego vehicle's ability to yield to EMS and avoid dangerous situations.
    ...
    def __init__(self, world, ego_vehicle, trigger_points=None):
        ...
        if trigger_points is not None:
            ...
            self.trigger_points = [(-67.50, -1.20),
                                   (-68.50, 2.20)]
        self.trigger_area = self.trigger_points
        self.checkpoints = [False, # Full throttle to gain speed, waypoint 7.20
                            False, # Reduced throttle to maintain speed, turn waypoint
                            False] # End of the turn, apply TrafficMnaager autopilot
        self.autopilot_frames = 0
        self.actor_list = []
        self.collision_detected = False
        self.random_yaw = random.choice([0, -170]) # Random yaw for the ambulance vehicle to turn right or left
        blueprint_library = world.get_blueprint_library()
        bp_ambulance = blueprint_library.find('vehicle.ford.ambulance')

        ambulance_transform = carla.Transform(carla.Location(x=-42.60, y=15.00, z=0.3), carla.Rotation(pitch=0, yaw=270, roll=0))
        
        self.ambulance_actor = world.spawn_actor(bp_ambulance, ambulance_transform)
        self.actor_list.append(self.ambulance_actor)
        self.ambulance_actor.set_light_state(carla.VehicleLightState(carla.VehicleLightState.Special1 | carla.VehicleLightState.LowBeam | carla.VehicleLightState.Fog | carla.VehicleLightState.Interior | carla.VehicleLightState.Position))
        self.collision_sensor = world.spawn_actor(blueprint_library.find('sensor.other.collision'), carla.Transform(), attach_to=self.ambulance_actor)
    ...
    def tick(self, ego_vehicle, traffic_manager):
        ...
        the scenario moves an EMS vehicle to either right or left.
        ...
        if self.triggered:
            if self.collision_detected:
                ...
                return 1
            ambulance_location = self.ambulance_actor.get_transform().location
            # Step 1, reduce throttle to maintain speed
            if not self.checkpoints[0]:
                if ambulance_location.y < 6.00:
                    self.ambulance_actor.apply_control(carla.VehicleControl(throttle=0.5, steer=0.0))
                    if self.random_yaw == 0:
                        self.ambulance_actor.set_light_state(carla.VehicleLightState(carla.VehicleLightState.RightBlinker | carla.VehicleLightState.Special1 | carla.VehicleLightState.LowBeam | carla.VehicleLightState.Fog | carla.VehicleLightState.Interior | carla.VehicleLightState.Position))
                    else:
                        self.ambulance_actor.set_light_state(carla.VehicleLightState(carla.VehicleLightState.LeftBlinker | carla.VehicleLightState.Special1 | carla.VehicleLightState.LowBeam | carla.VehicleLightState.Fog | carla.VehicleLightState.Interior | carla.VehicleLightState.Position))
                    self.checkpoints[0] = True
                else:
                    return 0
            # Step 2, turn waypoint
            elif not self.checkpoints[1]:
                if self.random_yaw == 0 and ambulance_location.y < 4.85:
                    self.ambulance_actor.apply_control(carla.VehicleControl(throttle=0.4, steer=1.0))
                    self.checkpoints[1] = True
                elif self.random_yaw == -170 and ambulance_location.y < 1.80:
                    self.ambulance_actor.apply_control(carla.VehicleControl(throttle=0.4, steer=-1.0))
                    self.checkpoints[1] = True
                else:
                    return 0
            # Step 3, end of the turn
            elif not self.checkpoints[2]:
                if self.random_yaw == 0:
                    if self.ambulance_actor.get_transform().rotation.yaw >= 0:
                        self.ambulance_actor.apply_control(carla.VehicleControl(throttle=0.6, steer=0.0))
                        self.checkpoints[2] = True
                else:
                    if self.ambulance_actor.get_transform().rotation.yaw <= -170:
                        self.ambulance_actor.apply_control(carla.VehicleControl(throttle=0.6, steer=0.0))
                        self.checkpoints[2] = True
                return 0
            # Step 4, apply TrafficManager autopilot
            # Set traffic manager for ambulance_actor vehicle to ignore traffic lights and speed limits and drive at +70% of the speed limit
            elif self.autopilot_frames < 100:
                if self.autopilot_frames == 0:
                    self.ambulance_actor.set_autopilot(True)
                    self.ambulance_actor.set_light_state(carla.VehicleLightState(carla.VehicleLightState.Special1 | carla.VehicleLightState.LowBeam | carla.VehicleLightState.Fog | carla.VehicleLightState.Interior | carla.VehicleLightState.Position))
                    traffic_manager.ignore_lights_percentage(self.ambulance_actor, 100)
                    traffic_manager.vehicle_percentage_speed_difference(self.ambulance_actor, -70)
                self.autopilot_frames += 1
                if self.autopilot_frames >= 100:
                    self.ambulance_actor.set_autopilot(False)  # Turn off autopilot after 100 frames
                    return 1
            else:
                self.ambulance_actor.apply_control(carla.VehicleControl(throttle=1.0, steer=0.0))
            return 0
    ...
    def trigger(self):
        ...
        Sets the `triggered` attribute to True, applies forward control to the EMS vehicle,
        ...
        self.triggered = True
        self.ambulance_actor.apply_control(carla.VehicleControl(throttle=1.0, steer=0.0))
        print('Scenario -Outgoing EMS- has been triggered!')
    ...
    def destroy(self, client):
        ...
        print('\nScenario -Outgoing EMS- finished!')
...
def main():
    ...
    try:
        ...
        # Spawn ego vehicle
        ego_vehicle_actor = world.spawn_actor(bp_ego_vehicle, carla.Transform(carla.Location(x=-77.00, y=0.50, z=0.3), carla.Rotation(pitch=0, yaw=0, roll=0)))
        ...
        try:
            while True:
                ...
                if Scenario is None:
                        Scenario = EMSOutgoing(world, ego_vehicle_actor)
                else:
                    if scenario_timer > 0:
