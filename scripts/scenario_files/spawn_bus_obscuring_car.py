...
class BusObscuringCar():
    """
    Represents a scenario where bus stops at bus stop and unloads kids, these kids run across the street.
    ...
    def __init__(self, world, ego_vehicle=None, trigger_points=None):
        ...
        if trigger_points is not None:
            ...
            self.trigger_points = [(-71.60, -185.00),
                                   (-75.60, -187.00)]
        ...
        bus_transform = carla.Transform(carla.Location(x=-45.40, y=-191.80, z=0.20), carla.Rotation(roll=0, pitch=0, yaw=1))
        vehicle_transform = carla.Transform(carla.Location(x=-35.80, y=-171.00, z=0.50), carla.Rotation(roll=0, pitch=0, yaw=270))
        ...
        for vehicle in [self.bus_actor, self.vehicle_actor]:
            vehicle.set_light_state(carla.VehicleLightState(carla.VehicleLightState.Interior | carla.VehicleLightState.Position | carla.VehicleLightState.LowBeam | carla.VehicleLightState.Fog))
            vehicle.apply_control(carla.VehicleControl(throttle=0.0, steer=0.0, brake=1.0, hand_brake=False, reverse=False))
            collision_sensor = world.spawn_actor(blueprint_library.find('sensor.other.collision'), carla.Transform(), attach_to=vehicle)
            self.actor_list.insert(0, collision_sensor)
            collision_sensor.listen(lambda event: self.sensor_callback(event))
    ...
    def tick(self, ego_vehicle, traffic_manager=None):
        ...
        if self.triggered:
            if self.collision_detected:
                print('Collision with a vehicle detected, scenario failed!')
                return 1
            vehicle_transform = self.vehicle_actor.get_transform()
            if self.checkpoint[0] == False:
                if vehicle_transform.location.y < (-197.00+get_vehicle_speed(self.vehicle_actor)):
                    self.vehicle_actor.apply_control(carla.VehicleControl(throttle=0.4, steer=1.0, brake=0.0, hand_brake=False, reverse=False))
                    self.checkpoint[0] = True
            elif self.checkpoint[1] == False:
                if vehicle_transform.rotation.yaw > -14.00:
    ...
    def trigger(self):
        ...
        self.vehicle_actor.apply_control(carla.VehicleControl(throttle=0.5, steer=0.0, brake=0.0, hand_brake=False, reverse=False))
        self.bus_actor.apply_control(carla.VehicleControl(throttle=0.0, steer=0.0, brake=1.0, hand_brake=False, reverse=False))
        self.triggered = True
        print('Scenario -Bus obscuring car- has been triggered!')
    ...
    def destroy(self, client):
        ...
        """
        for collision_sensor in [self.actor_list[0], self.actor_list[1]]:
            collision_sensor.stop()
        print('\nScenario -Bus obscuring car- finished!')
...
def main():
    ...
    try:
        ...
        # Spawn ego vehicle
        ego_vehicle_actor = world.spawn_actor(bp_ego_vehicle, carla.Transform(carla.Location(x=-73.60, y=-169.70, z=0.6), carla.Rotation(pitch=0, roll=0, yaw=272)))
        ...
        # !!! Success if the ego vehicle is slower !!!
        #traffic_manager.vehicle_percentage_speed_difference(ego_vehicle_actor, 5)
        ...
        try:
            while True:
                ...
                if Scenario is None:
                    Scenario = BusObscuringCar(world, ego_vehicle_actor)
                else:
                    if scenario_timer > 0:
