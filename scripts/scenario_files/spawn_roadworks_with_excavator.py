...
class RoadworksWithExcavator():
    """
    Represents a scenario where an excavator is stationary but there are iron planks placed on the road.

    This scenario spawns several actors, including a construction cone, street barriers, an excavator, a warning sign and iron planks.
    The objective of the scenario is to test the ego vehicle's behavior when encountering an unusual object placed on the road.
    ...
    def __init__(self, world, ego_vehicle=None, trigger_points=None):
        ...
        if trigger_points is not None:
            ...
            self.trigger_points = [(-118.50, 131.30),
                                   (-117.50, 134.50)]
        ...
        ironplank1_transform = carla.Transform(carla.Location(x=-136.00, y=127.20, z=0.0), carla.Rotation(pitch=0, yaw=310, roll=0))
        ironplank2_transform = carla.Transform(carla.Location(x=-137.00, y=128.40, z=0.0), carla.Rotation(pitch=0, yaw=310, roll=0))
        ...
        self.ironplank1_actor = world.spawn_actor(bp_ironplank, ironplank1_transform)
        self.actor_list.append(self.ironplank1_actor)
        self.ironplank2_actor = world.spawn_actor(bp_ironplank, ironplank2_transform)
        self.actor_list.append(self.ironplank2_actor)
    ...
    def tick(self, ego_vehicle, traffic_manager=None):
        ...
        the scenario checks if the ego vehicle speed is too low. If the speed is too low, the scenario is failed.
        The vešhicle is supposed to slow down and drive on the planks.
        ...
        if self.triggered:
            if self.timer > 0:
                # Check the speed of the ego vehicle
                if(get_vehicle_speed(ego_vehicle) < 0.05):
                    print('Scenario -Roadworks with Excavator- failed! Ego vehicle speed is too low!')
                    return 1
                else:
                    self.timer -= 1
                return 0
    ...
    def trigger(self):
        ...
        """
        self.triggered = True
        print('Scenario -Roadworks with Excavator- has been triggered!')
    ...
    def destroy(self, client):
        ...
        """
        print('\nScenario -Roadworks with Excavator- finished!')
    ...
    try:
        ...
        try:
            while True:
                ...
                if Scenario is None:
                    Scenario = RoadworksWithExcavator(world, ego_vehicle_actor)
                else:
                    if scenario_timer > 0:
