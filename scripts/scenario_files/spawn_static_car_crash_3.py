...
class StaticCarCrash_3():
    """
    Represents a scenario where a rear end car crash occures in front of ego vehicle before entering a roundabout.
    ...
    def __init__(self, world, ego_vehicle=None, trigger_points=None):
        ...
        bp_first_vehicle = blueprint_library.find('vehicle.mini.cooper_s_2021')
        bp_second_vehicle = blueprint_library.find('vehicle.lincoln.mkz_2020')

        first_vehicle_transform = carla.Transform(carla.Location(x=6.40, y=30.90, z=0.01), carla.Rotation(roll=0, pitch=0, yaw=285))
        second_vehicle_transform = carla.Transform(carla.Location(x=4.60, y=44.00, z=0.01), carla.Rotation(roll=0, pitch=0, yaw=270))

        self.first_car_actor = world.spawn_actor(bp_first_vehicle, first_vehicle_transform)
        self.actor_list.append(self.first_car_actor)
        self.second_car_actor = world.spawn_actor(bp_second_vehicle, second_vehicle_transform)
        self.actor_list.append(self.second_car_actor)
        for car in [self.first_car_actor, self.second_car_actor]:
            car.set_light_state(carla.VehicleLightState(carla.VehicleLightState.Interior | carla.VehicleLightState.Position | carla.VehicleLightState.LowBeam | carla.VehicleLightState.Fog))
            car.apply_control(carla.VehicleControl(throttle=0.0, steer=0.0, brake=1.0, hand_brake=False, reverse=False))
    ...
    def tick(self, ego_vehicle, traffic_manager=None):
        ...
        if self.triggered:
            if self.checkpoint[0] == False:
                if self.first_car_actor.get_transform().location.y < 29.20:
                    self.first_car_actor.set_light_state(carla.VehicleLightState(carla.VehicleLightState.Brake | carla.VehicleLightState.Fog | carla.VehicleLightState.Interior | carla.VehicleLightState.Position | carla.VehicleLightState.LowBeam))
                    self.first_car_actor.apply_control(carla.VehicleControl(throttle=0.0, steer=0.0, brake=1.0, hand_brake=False, reverse=False))
                    self.checkpoint[0] = True
            elif self.checkpoint[1] == False:
                if self.second_car_actor.get_transform().location.z > 0.0 or self.second_car_actor.get_transform().location.y < 33.45:
    ...
    def trigger(self):
        ...
        """
        self.triggered = True
        self.first_car_actor.apply_control(carla.VehicleControl(throttle=0.4, steer=0.01, brake=0.0, hand_brake=False, reverse=False))
        self.second_car_actor.apply_control(carla.VehicleControl(throttle=0.7, steer=0.05, brake=0.0, hand_brake=False, reverse=False))
    ...
    try:
        ...
        try:
            while True:
                ...
                if Scenario is None:
                    Scenario = StaticCarCrash_3(world, ego_vehicle_actor)
                else:
                    if scenario_timer > 0:
