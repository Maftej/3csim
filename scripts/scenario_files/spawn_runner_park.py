...
class RunnerInAPark():
    ...
    def __init__(self, world, ego_vehicle=None, trigger_points=None):
        ...
        if trigger_points is not None:
            ...
            self.trigger_points = [(-90.30, 8.50),
                                   (-110.80, 26.70)]
        self.trigger_area = self.trigger_points
        self.actor_list = []
        blueprint_library = world.get_blueprint_library()
        bp_pedestrian = random.choice(blueprint_library.filter('walker.pedestrian.' + str(random.randint(6, 8)).zfill(4)))

        if bp_pedestrian.has_attribute('is_invincible'):
            bp_pedestrian.set_attribute('is_invincible', 'false')
        
        pedestrian_transform = carla.Transform(carla.Location(x=-112.00, y=17.20, z=2.15), carla.Rotation(pitch=0, yaw=40, roll=0))
    ...
    def tick(self, ego_vehicle, traffic_manager=None):
        ...
        the pedestrian will start moving. The scenario is completed if the pedestrian
        is run over by the ego vehicle or if the ego vehicle successfully avoids the pedestrian.
    ...
    def trigger(self):
        ...
        """
        self.triggered = True
        self.pedestrian_actor.apply_control(carla.WalkerControl(direction=carla.Vector3D(x=1.0, y=1.0, z=0.1), speed=3.0, jump=False))
...
def main():
    ...
    try:
        ...
        # Spawn ego vehicle
        ego_vehicle_actor = world.spawn_actor(bp_ego_vehicle, carla.Transform(carla.Location(x=-101.30, y=0.20, z=0.3), carla.Rotation(pitch=0, yaw=0, roll=0)))
        ...
        try:
            while True:
                ...
                if Scenario is None:
                    Scenario = RunnerInAPark(world, ego_vehicle_actor)
                else:
                    if scenario_timer > 0:
