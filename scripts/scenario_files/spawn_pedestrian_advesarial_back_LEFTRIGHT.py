...
# Original scenario file edited to deterministically spawn a pedestrian
# with a "turn left or right only" traffic sign on his shirt or jacket seen from the back
...
class PedestrianAdvesarialBack():
    ...
    def __init__(self, world, ego_vehicle=None, trigger_points=None):
        ...
        bp_pedestrian = blueprint_library.find('walker.pedestrian.0005')

        if bp_pedestrian.has_attribute('is_invincible'):
            bp_pedestrian.set_attribute('is_invincible', 'false')
        
        pedestrian_transform = carla.Transform(carla.Location(x=135.40, y=-79.25, z=9.15), carla.Rotation(roll=0, pitch=0, yaw=182))
    ...
    def trigger(self):
        ...
        """
        self.pedestrian_actor.apply_control(carla.WalkerControl(direction=carla.Vector3D(x=-1.0, y=-0.02, z=0.1), speed=3.0, jump=False))
        self.triggered = True
