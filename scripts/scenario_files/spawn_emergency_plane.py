...
class EmergencyPlane():
    def __init__(self, world, ego_vehicle=None, trigger_points=None):
        ...
        cirrus_body_transform = carla.Transform(carla.Location(x=209.50, y=205.30, z=23.00), carla.Rotation(pitch=0, yaw=180, roll=0))
    ...
    def tick(self, ego_vehicle, traffic_manager=None):
        if self.triggered:
            ...
            if self.checkpoint == False:
                if current_location.z > 3.60:
                    self.cirrus_body_actor.set_transform(carla.Transform(carla.Location(x=current_location.x - 0.95, y=current_location.y, z=current_location.z - 0.20), carla.Rotation(pitch=0, yaw=180, roll=0)))
                else:
                    self.cirrus_body_actor.set_simulate_physics(True)
