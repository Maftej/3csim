...
# Original scenario file edited using traffic manager speed control to avoid collision
...
class CarForwardStoppingDropoff():
    ...
    The objective of the scenario is to test ego vehicle prediction of vehicle maneuvers on the road.
    ...
    def __init__(self, world, ego_vehicle=None, trigger_points=None):
        ...
        if trigger_points is not None:
            ...
            self.trigger_points = [(3.60, 66.00),
                                   (7.10, 68.00)]
        ...
        if self.triggered:
            ...
            if self.checkpoint[0] == False:
                if vehicle_transform.rotation.yaw > -60.00:
                    self.vehicle_actor.apply_control(carla.VehicleControl(throttle=0.4, steer=-0.6, brake=0.0, hand_brake=False, reverse=False))
                ...
                if self.timer > 50 and ego_vehicle.get_transform().location.y > 74.50:
                    self.timer -= 1
                else:
...
def main():
    ...
    try:
        ...
        traffic_manager.vehicle_percentage_speed_difference(ego_vehicle_actor, 5.0)
