...
class CarReverseParkingWithDoors():
    """
    Represents a scenario where the vehicle parks in a parking lane too close to the lane and opens the doors when the ego vehicle arrives.
    ...
    def __init__(self, world, ego_vehicle=None, trigger_points=None):
        ...
        if trigger_points is not None:
            ...
            self.trigger_points = [(152.60, -145.30),
                                   (156.20, -142.70)]
        ...
        vehicle_transform = carla.Transform(carla.Location(x=154.50, y=-170.00, z=1.90), carla.Rotation(roll=0, pitch=-12, yaw=270))
    ...
    def tick(self, ego_vehicle, traffic_manager=None):
        ...
        If the scenario is triggered and the ego vehicle enters the trigger area,
        the scenario moves the vehicle into a parking lane and straightens the vehicle.
        When ego vehicle arrives, the vehicle opens the front left door and the scenario is completed.
        ...
        if self.triggered:
            ...
            if self.checkpoint[0] == False:
                if vehicle_transform.rotation.yaw < -130.00:
                    self.vehicle_actor.apply_control(carla.VehicleControl(throttle=0.6, steer=-0.6, brake=0.0, hand_brake=False, reverse=True))
                    self.checkpoint[0] = True
            elif self.checkpoint[1] == False:
                if vehicle_transform.rotation.yaw > -93.00:
                    self.vehicle_actor.apply_control(carla.VehicleControl(throttle=0.0, steer=-0.2, brake=0.0, hand_brake=True, reverse=False))
                ...
                if ego_vehicle.get_transform().location.y < -157.60:
                    self.vehicle_actor.open_door(carla.VehicleDoor.FL)
    ...
    def trigger(self):
        ...
        Sets the `triggered` attribute to True, moves the vehicle to the right parking lane,
        applies right blinker, and prints a message indicating that the scenario has been triggered.
        Can be used to trigger the scenario at a specific point in the simulation.
        """
        self.vehicle_actor.apply_control(carla.VehicleControl(throttle=0.8, steer=0.6, brake=0.0, hand_brake=False, reverse=True))
        self.vehicle_actor.set_light_state(carla.VehicleLightState(carla.VehicleLightState.Reverse | carla.VehicleLightState.RightBlinker | carla.VehicleLightState.Fog | carla.VehicleLightState.Interior | carla.VehicleLightState.Position | carla.VehicleLightState.LowBeam))
        self.triggered = True
        print('Scenario -Car reverse parking and opens doors- has been triggered!')
    ...
    def destroy(self, client):
        ...
        print('\nScenario -Car reverse parking and opens doors- finished!')
...
def main():
    ...
    try:
        ...
        # Spawn ego vehicle
        ego_vehicle_actor = world.spawn_actor(bp_ego_vehicle, carla.Transform(carla.Location(x=153.80, y=-117.90, z=8.2), carla.Rotation(pitch=0, roll=0, yaw=270)))
        ...
        try:
            while True:
                ...
                if Scenario is None:
                    Scenario = CarReverseParkingWithDoors(world, ego_vehicle_actor)
                else:
                    if scenario_timer > 0:
