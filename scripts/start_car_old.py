...
import carla
from scenario_files import BoyFootball, FootballHighway, EMSOutgoing, CarWithItem, is_inside_bounding_box

scenarios = [BoyFootball, FootballHighway, EMSOutgoing, CarWithItem]
...
def main():
    ...
    argparser.add_argument(
        ...
        help='Set the maximum time for the scenario to run in range 200-2000 seconds (default: 600)')
    argparser.add_argument(
        '-rgb', '--rgbcamera',
        metavar='RGB',
        default=True,
    ...
    try:
        ...
        vehicle_path = ['Right', 'Right', 'Straight', 'Straight', 'Right', 'Straight', 'Straight', 'Right', 'Straight', 'Right', 'Straight']
        ...
        try:
            while True:
                ...
                if Scenario is None:
                    # Check if ego_vehicle_actor is inside the bounding box, if yes, trigger the scenario
                    if is_inside_bounding_box(ego_vehicle_actor.get_transform().location, bounding_box_kid[0], bounding_box_kid[1]):
                        Scenario = BoyFootball(world, ego_vehicle_actor)
                        print('Scenario -Boy With a Ball- has been triggered!')
                        print(ego_vehicle_actor.get_transform().location)
                    if is_inside_bounding_box(ego_vehicle_actor.get_transform().location, bounding_box_ball[0], bounding_box_ball[1]):
                        Scenario = FootballHighway(world, ego_vehicle_actor)
                        print('Scenario -Football on Highway- has been triggered!')
                        print(ego_vehicle_actor.get_transform().location)
                    if is_inside_bounding_box(ego_vehicle_actor.get_transform().location, bounding_box_EMS_outgoing[0], bounding_box_EMS_outgoing[1]):
                        Scenario = EMSOutgoing(world, ego_vehicle_actor)
                        print('Scenario -EMS Outgoing- has been triggered!')
                        print(ego_vehicle_actor.get_transform().location)
                    if is_inside_bounding_box(ego_vehicle_actor.get_transform().location, bounding_box_EMS_outgoing[0], bounding_box_EMS_outgoing[1]):
